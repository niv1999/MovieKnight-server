...
- **`sort`** — allowable values: `popularity` *(default; all-time most-rated/viewed → `vote_count.desc`)*, `trending` *(most viewed/searched this week → TMDB `/trending/movie/week`)*, `rating_desc`, `rating_asc`, `title_asc`, `title_desc`, `year_desc`, `year_asc`. Unknown/missing → `popularity`. The two popularity-style sorts are **genuinely different feeds**: `popularity` = `vote_count.desc` surfaces all-time greats (Fight Club, Pulp Fiction, Inception, …); it is **not** TMDB's `popularity.desc`, which is a recency-weighted "hot right now" score that only returns the current year's releases. `trending` is what's most viewed this week. On the discover path the sort is native via TMDB `sort_by` (title → `original_title.*`, year → `primary_release_date.*`). `sort=trending` is a feed **mode**: it routes to `/trending/movie/week` (TMDB has only day/week trending windows — there is no "all-time trending"; all-time is `popularity`). **All filters work on both sorts.** `/trending` can't filter natively, so every filter is re-applied server-side in-memory on the trending page: `genre`/`yearFrom`/`yearTo`/`minRating`/`minVotes`/`language` cheaply off the result objects, and `certification`/`providers`/`with_cast`/`with_crew` via a per-title detail fetch (`/movie/{id}` with `append_to_response=release_dates,credits,watch/providers`, run concurrently after the cheap filters narrow the page). Trending order is preserved; a page can shrink as filters trim it. A free-text query (`q`) ignores `sort` entirely (results come back in `/search/movie` relevance order). Undated titles sort to the bottom on `year_*` and are excluded when a `yearFrom`/`yearTo` range is set.
...
- **Errors:** `400` (bad/missing input) · `401` (no/invalid token) · `404` (collection missing or not the caller's) · `500` (AI service not configured) · `502` (the model returned malformed/non-JSON, or an upstream/quota failure) · `504` (the AI call exceeded the timeout, default 20s). Client-facing error messages are **provider-masked** — they say "AI service …", never naming Gemini or echoing the raw SDK/quota detail (that's logged server-side only, mirroring the TMDB client). Requires `GEMINI_API_KEY` in the server env (see `.env.example`); optional `GEMINI_MODEL` / `GEMINI_TIMEOUT_MS` overrides.
