...
|---|---|---|---|---|
| GET | `/api/movies/search` | `q, genre, yearFrom, yearTo, minRating, minVotes, language, with_cast, with_crew, providers, watch_region, certification, certification_country, sortBy, page` | `{ ok:true, data:[movie] }` | ✅ implemented |
| GET | `/api/movies/random` | `pages` (optional) | `{ ok:true, data:{movie object} }` | ✅ implemented |
...
- **`sortBy`** — these are **semantic intent names**; the client sends an intent and never a TMDB sort string. The server owns the TMDB mapping, so the retrieval logic can change without touching the client. Allowable values: `popularity` *(default)*, `trending`, `rating_desc`, `rating_asc`, `title_asc`, `title_desc`, `year_desc`, `year_asc`. Unknown/missing → `popularity`. Server-side mapping to TMDB `/discover` `sort_by`:
  - `popularity` → `vote_count.desc` — all-time most-rated/viewed (Fight Club, Pulp Fiction, Inception, …). The "Most Popular" / overall feed.
  - `trending` → `popularity.desc` — TMDB's recency-weighted "hot right now" score (what's currently being watched/searched). The "Popular this week" feed.
  - `rating_desc`/`rating_asc` → `vote_average.*`, `title_*` → `original_title.*`, `year_*` → `primary_release_date.*`.

  The two popularity-style sorts are **genuinely different feeds** and **all filters work on both** — every non-search request goes through `/discover`, so all filters apply natively (no per-title lookups, no page shrinking). A free-text query (`q`) ignores `sortBy` entirely (results come back in `/search/movie` relevance order). Undated titles sort to the bottom on `year_*` and are excluded when a `yearFrom`/`yearTo` range is set.
...
- Returns one random — but recognizable — movie for the home "Surprise Me" button. The server picks a random title from a random page of the popular, well-voted, English discover feed (`sort_by=vote_count.desc`, `vote_count.gte=500`, `with_original_language=en` — the **same pool as the default feed**), so picks are always mainstream rather than obscure. `{ ok:true, data:{movie object} }`.
- **`pages`** (optional) — how many pages of that pool to randomize from (default **50** ≈ top ~1,000 titles; clamped to TMDB's `1–500`). A bigger value widens the pool toward more obscure picks. The client will later derive this from a per-user setting (movies-to-randomize-from ÷ 20); for now it sends no param and gets the default.
...
- **`card`** (list / create / patch): `{ id, name, isDefault, isPublic, posterUrl, movieCount, movieIds, covers, likesCount, savesCount, author, isOwner, createdAt }`. `covers` = the first ≤4 movies as `{ poster, backdrop }` bare TMDB paths (client prefixes the CDN and picks poster **or** backdrop per cover layout — which depends on the movie count **and** viewport; see `buildCollectionCover`). `movieIds` = the **full** id list for cheap membership checks (drives the home/movie heart & eye); `posterUrl` (custom cover) overrides the generated cover when set.
...
- **Default lists** (`isDefault:true`) can't be **renamed** or **deleted** (`400`), but their **visibility can** be toggled. `POST /:id/movies` is **idempotent** (re-adding a movie is a no-op) and warms the `movies` cache (poster + backdrop) so the cover/grid always has its images.
...
- `wheelConfig` is stored as the embedded `collection.savedWheel` (`[String]`). PUT body is `{ wheelConfig: [string] }`; entries are trimmed, empties dropped, capped at 100.
